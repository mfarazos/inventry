...
import { getPurchaseLedgerSummary } from "@/services/GameManagement";
...
const getVendorName = (item: PurchaseLedgerRow) => {
  return (
    item.vendorName ||
    item.receivedFrom ||
...
const getVendorRef = (item: PurchaseLedgerRow) => {
  return item.vendorRef || item.vendor?.vendorRef || "-";
};

const getClientTypeLabel = (item: PurchaseLedgerRow) => {
  const rowUserType = String(item.userType || item.vendor?.userType || "")
    .trim()
    .toLowerCase();
...
const getClientTypeFilterLabel = (value: string) => {
  if (value === "walkingCustomer") return "Walking Vendors";
  if (value === "specificCustomer") return "Extruding Vendors";
...
const PurchaseLedgerSummary = () => {
  ...
  const [ledgerData, setLedgerData] = useState<PurchaseLedgerRow[]>([]);
  const [summary, setSummary] = useState<PurchaseLedgerTotals | null>(null);
  ...
  const displayTotalWeight = summary?.totalWeight ?? 0;
  const displayClosingBalance =
    summary?.closingBalance ?? summary?.finalBalance ?? 0;

  const fetchPurchaseLedgerSummary = useCallback(async () => {
    try {
      setLoading(true);

      const response = await getPurchaseLedgerSummary({
        ...requestParams,
        _t: Date.now(),
      });
      ...
      console.log("Purchase ledger summary error", error);
      setLedgerData([]);
      setSummary(null);
      toast.push(
        <Notification title="Unable to fetch purchase ledger" type="danger">
          Please try again.
        </Notification>,
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const filename = `purchase-ledger-summary-${periodLabel}-${materialLabel}-${clientLabel}`
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-|-$/g, "");
      const rows = ledgerData.length
        ? ledgerData
            .map((item, index) => {
              ...
              return `
                <tr class="${rowType === "normal" ? "" : `${rowType}-row`}">
                  ...
                  <td class="center">${getVendorRef(item)}</td>
                  <td class="center">${getClientTypeLabel(item)}</td>
                  <td class="description">${item.description || "-"}</td>
                  <td class="center">${item.billNo || "-"}</td>
                  <td class="center">${item.folio || "-"}</td>
                  <td class="amount">${item.totalWeight ? formatAmount(item.totalWeight) : "-"}</td>
            ...
            .join("")
        : '<tr><td colspan="11" class="empty">No purchase ledger summary found</td></tr>';

      element = document.createElement("div");
      element.innerHTML = `
        <style>
          .purchase-ledger-pdf { padding: 24px; font-family: Arial, sans-serif; color: #1f2937; background: #fff; }
          ...
          col.date { width: 7%; } col.vendor { width: 13%; } col.ref { width: 7%; } col.type { width: 4%; }
          col.description { width: 28%; } col.short { width: 6%; } col.amount { width: 7.25%; }
          th, td { border: 1px solid #d8dee7; padding: 6px 4px; vertical-align: top; word-break: break-word; }
          th { background: #f1f5f9; text-align: center; text-transform: uppercase; font-size: 7px; }
        ...
        <div class="purchase-ledger-pdf">
          <div class="pdf-header">
            <div>
              <h1>Purchase Ledger Summary</h1>
          ...
          ${
            isAdmin
              ? `<div class="summary-grid">
                  ...
                  <div class="summary-item"><span>Total Weight</span><strong>${formatAmount(displayTotalWeight)}</strong></div>
          ...
          <table>
            <colgroup>
              <col class="date"/><col class="vendor"/><col class="ref"/><col class="type"/><col class="description"/>
              <col class="short"/><col class="short"/><col class="amount"/><col class="amount"/><col class="amount"/><col class="amount"/>
            </colgroup>
            <thead><tr><th>Date</th><th>Vendor Name</th><th>Vendor Ref</th><th>Type</th><th>Description</th><th>Bill No</th><th>Folio</th><th>Weight</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
      `;
      ...
      console.log("Purchase ledger PDF error", error);
      toast.push(
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h3 className="text-gray-900">Purchase Ledger Summary</h3>
          <p className="mt-1 text-sm text-gray-500">
            Overall purchase ledger for {getClientTypeFilterLabel(clientType).toLowerCase()} - {periodLabel}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-[170px]">
            ...
            >
              <option value="all">All</option>
              <option value="walkingCustomer">Walking Vendor</option>
              <option value="specificCustomer">Extruding Vendor</option>
            </select>
          </div>
          ...
          <Button
            ...
            variant="default"
            loading={loading}
            onClick={fetchPurchaseLedgerSummary}
          >
            Refresh
          </Button>
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-5">
          ...
          <div className="rounded border border-gray-200 bg-white px-4 py-3">
            ...
            <p className="mt-1 text-lg font-bold text-slate-800">
              {formatAmount(displayTotalWeight)}
            </p>
          </div>
          <div className="rounded border border-gray-200 bg-white px-4 py-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">
      ...
      <div className="rounded border border-gray-200 bg-white">
        ...
        <div className="overflow-x-auto">
          <Table className="min-w-[1320px] table-fixed" hoverable={false}>
            <colgroup>
              <col className="w-[7%]" />
              <col className="w-[13%]" />
              <col className="w-[7%]" />
            ...
            <THead>
              <Tr className="bg-gray-50">
                <Th className="!text-left text-xs uppercase tracking-wide text-gray-500">Date</Th>
                <Th className="!text-left text-xs uppercase tracking-wide text-gray-500">Vendor Name</Th>
                <Th className="!text-center text-xs uppercase tracking-wide text-gray-500">Vendor Ref</Th>
                <Th className="!text-center text-xs uppercase tracking-wide text-gray-500">Type</Th>
                <Th className="!text-left text-xs uppercase tracking-wide text-gray-500">Description</Th>
                <Th className="!text-center text-xs uppercase tracking-wide text-gray-500">Bill No</Th>
                <Th className="!text-center text-xs uppercase tracking-wide text-gray-500">Folio</Th>
                <Th className="!text-right text-xs uppercase tracking-wide text-gray-500">Weight</Th>
                <Th className="!text-right text-xs uppercase tracking-wide text-gray-500">Debit</Th>
                <Th className="!text-right text-xs uppercase tracking-wide text-gray-500">Credit</Th>
                <Th className="!text-right text-xs uppercase tracking-wide text-gray-500">Balance</Th>
              </Tr>
            </THead>
              ...
              {loading ? (
                <Tr>
                  <Td colSpan={11} className="py-8 text-center text-gray-500">
                    Loading purchase ledger summary...
                  </Td>
                </Tr>
              ) : ledgerData.length > 0 ? (
                ledgerData.map((item, index) => {
                  ...
                  return (
                    <Tr key={`${item.entryType || "row"}-${index}`} className={rowClassName}>
                      <Td className="whitespace-nowrap !text-left text-gray-900">{formatDate(item.date)}</Td>
                      <Td className="!text-left text-gray-800">{getVendorName(item)}</Td>
                      <Td className="!text-center text-gray-700">{getVendorRef(item)}</Td>
                      ...
                      <Td className="whitespace-normal break-words !text-left leading-5">{item.description || "-"}</Td>
                      <Td className="!text-center text-gray-700">{item.billNo || "-"}</Td>
                      <Td className="!text-center text-gray-700">{item.folio || "-"}</Td>
                      <Td className="!text-right font-medium tabular-nums text-slate-700">
                        {item.totalWeight ? formatAmount(item.totalWeight) : "-"}
                      </Td>
                ...
                <Tr>
                  <Td colSpan={11} className="py-8 text-center text-gray-500">
                    No purchase ledger summary found for {periodLabel}
                  </Td>
                </Tr>
              )}
            </TBody>
...
export default PurchaseLedgerSummary;
